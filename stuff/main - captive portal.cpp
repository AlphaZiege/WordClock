...
void setup()
{
  ...
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest *request) { // hört auf /data und macht dann das
    ...
    else if (inputName == "wlan1_ssid")
    {
      settings.set_wlan_ssid(inputVar);
      request->send(200, "text/plain", inputName + "set to: " + inputVar);
    }
    else if (inputName == "wlan2_ssid")
    {
      settings.set_wlan2_ssid(inputVar);
      request->send(200, "text/plain", inputName + "set to: " + inputVar);
    }
    else if (inputName == "wlan3_ssid")
    {
      settings.set_wlan3_ssid(inputVar);
      request->send(200, "text/plain", inputName + "set to: " + inputVar);
    }
    else if (inputName == "wlan1_pw")
    {
      settings.set_wlan_ssid(inputVar);
      String var;
    ...
    else if (inputName == "save_wlan1")
    {
      storage.saveWlan1();
    }
    else if (inputName == "save_wlan2")
    {
      storage.saveWlan2();
    }
    else if (inputName == "save_wlan3")
    {
      storage.saveWlan3();
    }
