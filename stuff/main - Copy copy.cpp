...
// Wlan Zugang
const char *ssid = "JJL_FRITZ!Box 7590 2,4";
const char *password = "72054378159759666929";
...
void setup()
{
  ...
  server.on("/data", HTTP_GET, [](AsyncWebServerRequest *request) { // hört auf /data und macht dann das
    ...
    {
      settings.set_fadeColor(false);
      settings.set_randomColor(false);
      settings.set_staticColor(true);
    }
    else if (inputName == "fadeColor")
    {
      settings.set_randomColor(false);
      settings.set_staticColor(false);
      settings.set_fadeColor(true);
    }
    else if (inputName == "randomColor")
    {
      settings.set_fadeColor(false);
      settings.set_staticColor(false);
      settings.set_randomColor(true);
    }
    else if (inputName == "staticColor_mode")
    {
      if (input.toInt() == 0)
      {
        settings.set_randomColor_mode(false);
      }
      else if (input.toInt() == 1)
      {
        settings.set_randomColor_mode(true);
      }
    }
...
void loop()
{
  ...
  //lichteffekte
  if (settings.get_staticColor()) //statische Farbe mit sc col
  {
  ...
  if (settings.get_fadeColor()) //breathe mit sc col
  {
    for (int i = 0; i < led_count; i++)
      strip.setPixelColor(i, settings.get_SC_RED(), settings.get_SC_GREEN(), settings.get_SC_BLUE());
    if (millis() >= timestamp + 5)
    {
      if (fade_in)
      {
        ...
        for (int b = 0; b < led_count; b++)
          strip.setPixelColor(b, settings.get_SC_RED(), settings.get_SC_GREEN(), settings.get_SC_BLUE());
        breathe_index++;
  ...
  if (settings.get_randomColor()) //zufällige Farbe die sich viel zu schnell ändern
  {
